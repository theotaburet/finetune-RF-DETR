...
class DownloadConfig:
    ...
    def from_yaml(cls, path: Path) -> DownloadConfig:
        ...
        # Fall back to environment variable if token is not set in config file
        if not config.api.token:
            config.api.token = os.environ.get("EKB_API_TOKEN")
        if "output" in data:
...
class DataDownloader:
    ...
    def _filter_sounds(self, sounds: list[SoundSegment]) -> list[SoundSegment]:
        """Apply post-grouping sound-level filters and cap to max_sounds.

        Filters are applied in this order:
          1. min_confidence  — keep sounds whose highest-confidence event meets the threshold
          2. min_duration_s  — keep sounds at least this long
          3. max_duration_s  — keep sounds at most this long
          4. max_sounds      — randomly sample down to this many (using the configured seed)

        Args:
            sounds: All grouped sound segments.

        Returns:
            Filtered (and possibly truncated) list of sound segments.

        """
        limits = self.config.limits
        original = len(sounds)

        if limits.min_confidence is not None:
            sounds = [s for s in sounds if any(e.confidence >= limits.min_confidence for e in s.events)]
            logger.debug(
                "After min_confidence=%.2f filter: %d / %d sounds", limits.min_confidence, len(sounds), original
            )

        if limits.min_duration_s is not None:
            sounds = [s for s in sounds if s.duration_s >= limits.min_duration_s]
            logger.debug("After min_duration_s=%.2f filter: %d sounds", limits.min_duration_s, len(sounds))

        if limits.max_duration_s is not None:
            sounds = [s for s in sounds if s.duration_s <= limits.max_duration_s]
            logger.debug("After max_duration_s=%.2f filter: %d sounds", limits.max_duration_s, len(sounds))

        if limits.max_sounds is not None and len(sounds) > limits.max_sounds:
            rng = np.random.default_rng(self.config.advanced.seed)
            indices = rng.choice(len(sounds), size=limits.max_sounds, replace=False)
            sounds = [sounds[i] for i in sorted(indices)]
            console.print(
                f"  [dim]Capped to {limits.max_sounds} sounds (max_sounds limit, "
                f"seed={self.config.advanced.seed})[/dim]"
            )

        if len(sounds) < original:
            console.print(f"  [dim]Sound filters reduced {original} → {len(sounds)} sounds[/dim]")
    ...
    def run(self) -> list[DownloadResult]:
        ...
        # Apply post-grouping sound-level filters (confidence, duration, max_sounds cap)
        all_sounds = self._filter_sounds(all_sounds)
...
def merge_config_with_args(config: DownloadConfig, args: argparse.Namespace) -> DownloadConfig:
    ...
    if hasattr(args, "max_sounds") and args.max_sounds is not None:
        config.limits.max_sounds = args.max_sounds
    if hasattr(args, "min_confidence_sound") and args.min_confidence_sound is not None:
        config.limits.min_confidence = args.min_confidence_sound
    if hasattr(args, "min_duration_s") and args.min_duration_s is not None:
        config.limits.min_duration_s = args.min_duration_s
    if hasattr(args, "max_duration_s") and args.max_duration_s is not None:
        config.limits.max_duration_s = args.max_duration_s
...
Examples:
  ...
  python run_download_data.py --api-url https://api.example.com/ekb/api \\
    ...
    parser.add_argument(
        "--max-labels",
        type=int,
        help="Maximum number of labels to fetch from the API",
    )
    parser.add_argument(
        "--max-sounds",
        type=int,
        help="Maximum number of merged sound segments to download (randomly sampled, reproducible via --seed)",
    )
    parser.add_argument(
        "--min-confidence-sound",
        type=float,
        metavar="CONFIDENCE",
        help="Keep only sounds where at least one event has confidence >= this value (0.0–1.0)",
    )
    parser.add_argument(
        "--min-duration-s",
        type=float,
        metavar="SECONDS",
        help="Keep only sounds with duration >= this value in seconds",
    )
    parser.add_argument(
        "--max-duration-s",
        type=float,
    ...
    if args.config:
        ...
        # Start with default config, apply env var for token
        config = DownloadConfig()
        if not config.api.token:
            config.api.token = os.environ.get("EKB_API_TOKEN")
